...
export default function App() {
  ...
  const handleJsonFileChange = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Suporta tanto JSONL (linhas de JSON) quanto JSON padrão
  ...
  const handleProcessImportedCampaign = async (campaignData) => {
    ...
    for (let i = 0; i < importedJsonLeads.length; i++) {
      ...
      leads.push({
        ...
        pitch: aiData?.pitch || `Olá pessoal da ${name}, tudo bem? Localizei o vosso perfil nas pesquisas do Google Maps. Conseguimos conversar por chamada amanhã?`,
        // DADOS REAIS TRAVADOS E BLINDADOS DO FICHEIRO:
  ...
  const getWhatsAppLink = (phoneNum, pitchText) => {
    if (!phoneNum || phoneNum === 'não achou' || phoneNum === 'não listado' || phoneNum === 'não encontrado' || phoneNum === 'Não informado') return null;
    ...
    <div className="min-h-screen text-slate-200 font-sans pb-24 relative overflow-hidden flex bg-[#141417]">
      ...
      <aside className="w-64 bg-[#09090B] border-r border-[#27272A] flex flex-col justify-between p-6 flex-shrink-0 min-h-screen">
            ...
            {[
              ...
              return (
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold tracking-wide transition-all ${isActive ? 'bg-[#18181B] text-white border border-[#27272A] shadow-sm' : 'text-slate-400 hover:text-slate-200 hover:bg-[#18181B]/50 border border-transparent'}`}
                >
        ...
        <div className="space-y-6">
          <button onClick={() => setIsProfileModalOpen(true)} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-slate-400 hover:text-white transition-all hover:bg-[#18181B]/50">
            <Settings size={16} />
      ...
      <main className="flex-1 max-w-5xl mx-auto px-8 py-10 overflow-y-auto">
        ...
        {currentView === 'dashboard' && (
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { label: 'Empresas Encontradas', value: campaigns.flatMap(c => c.leads || []).length, desc: 'Alvos potenciais na base de dados', icon: Building2 },
                { label: 'Abordagens Iniciadas', value: campaigns.flatMap(c => c.leads || []).filter(l => l.status !== 'não contatado').length, desc: 'Contatos ativos estabelecidos', icon: MessageSquare },
                { label: 'Conversão Comercial', value: `${campaigns.flatMap(c => c.leads || []).filter(l => l.status !== 'não contatado').length > 0 ? Math.round((campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido').length / campaigns.flatMap(c => c.leads || []).filter(l => l.status !== 'não contatado').length) * 100) : 0}%`, desc: 'Rácio de fechamento sobre contatos', icon: BarChart2 }
              ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* Ajustável Meta Semanal Financeira */}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-3xl font-extrabold text-emerald-400 tracking-tight">
                        {formatCurrency(campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido' && isDateInCurrentWeek(l.conversionDate)).reduce((sum, l) => sum + (Number(l.dealValue) || 0), 0))}
                      </span>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-xs font-bold text-slate-400">
                      ...
                      <span>{weeklyGoal > 0 ? Math.min(100, Math.round((campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido' && isDateInCurrentWeek(l.conversionDate)).reduce((sum, l) => sum + (Number(l.dealValue) || 0), 0) / weeklyGoal) * 100)) : 0}%</span>
                    </div>
                    <div className="w-full bg-[#18181B] border border-[#27272A] h-3 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${weeklyGoal > 0 ? Math.min(100, Math.round((campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido' && isDateInCurrentWeek(l.conversionDate)).reduce((sum, l) => sum + (Number(l.dealValue) || 0), 0) / weeklyGoal) * 100)) : 0}%` }} 
                      />
                    </div>
                  </div>
                  <p className="text-[11px] text-[#9CA3AF] font-semibold">Gerado por {campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido' && isDateInCurrentWeek(l.conversionDate)).length} contrato(s) fechado(s) esta semana.</p>
                  {campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido' && isDateInCurrentWeek(l.conversionDate)).reduce((sum, l) => sum + (Number(l.dealValue) || 0), 0) >= weeklyGoal ? (
                    ...
                    <div className="text-[11px] text-[#9CA3AF] font-medium">Faltam {formatCurrency(Math.max(0, weeklyGoal - campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido' && isDateInCurrentWeek(l.conversionDate)).reduce((sum, l) => sum + (Number(l.dealValue) || 0), 0)))} para atingir a meta semanal.</div>
                  )}
                </div>
        ...
        {/* VIEW: EMPRESAS (LISTAGEM INTEGRADA) */}
        {currentView === 'leads' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row gap-4 bg-[#09090B] border border-[#27272A] p-4 rounded-2xl">
            ...
            <div className="border border-[#27272A] bg-[#09090B] rounded-2xl overflow-hidden">
              <table className="w-full text-left font-sans text-sm">
                ...
                <tbody className="divide-y divide-[#27272A]">
                  {campaigns.flatMap(c => (c.leads || []).map(l => ({ ...l, campaignId: c.id, campaignName: c.name })))
                    .filter(lead => {
        ...
        {/* VIEW: ROTEIROS DE ABORDAGEM */}
        {currentView === 'abordagens' && (
          <div className="space-y-6 animate-fade-in">
            <div className="pb-4 border-b border-[#27272A]">
            ...
            {campaigns.flatMap(c => c.leads || []).length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {campaigns.flatMap(c => (c.leads || []).map(l => ({ ...l, campaignId: c.id })))
                  .map(lead => (
                      ...
                      {selectedLeadForTone === lead.id && (
                        <div className="bg-[#18181B] border border-[#27272A] p-4 rounded-2xl space-y-3 animate-fade-in">
                          ...
                          <div className="flex gap-2">
                            <input type="text" placeholder="Ex: Mudar para um tom mais curto..." className="flex-1 px-4 py-2 bg-[#09090B] border border-[#27272A] rounded-xl text-xs text-white focus:outline-none" value={aiToneCommand} onChange={e => setAiToneCommand(e.target.value)} />
        ...
        {currentView === 'receita' && (
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { label: 'Faturamento Acumulado', value: formatCurrency(campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido').reduce((sum, l) => sum + (Number(l.dealValue) || 0), 0)), desc: 'Soma total dos contratos fechados', icon: DollarSign, color: 'text-emerald-400' },
                { label: 'Faturamento da Semana', value: formatCurrency(campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido' && isDateInCurrentWeek(l.conversionDate)).reduce((sum, l) => sum + (Number(l.dealValue) || 0), 0)), desc: 'Ganhos gerados na semana atual', icon: TrendingUp, color: 'text-[#C5A27D]' },
                { label: 'Ticket Médio', value: formatCurrency(campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido').length > 0 ? Math.round(campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido').reduce((sum, l) => sum + (Number(l.dealValue) || 0), 0) / campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido').length) : 0), desc: 'Valor médio por contrato assinado', icon: Clipboard, color: 'text-indigo-400' }
              ].map((card, i) => (
                <div key={i} className="border border-[#27272A] bg-[#09090B] p-6 rounded-2xl flex justify-between items-start">
            ...
            <div className="border border-[#27272A] bg-[#09090B] rounded-2xl overflow-hidden">
              <div className="p-6 border-b border-[#27272A] flex items-center justify-between">
                ...
                <span className="text-xs font-semibold text-slate-400 bg-[#18181B] border border-[#27272A] px-3 py-1 rounded-full">{campaigns.flatMap(c => c.leads || []).filter(l => l.status === 'convertido').length} conversões</span>
              </div>
              <table className="w-full text-left font-sans text-sm">
                <thead>
                  <tr className="text-[#9CA3AF] font-semibold text-xs uppercase border-b border-[#27272A] bg-[#09090B]">
                ...
                <tbody className="divide-y divide-[#27272A]">
                  {campaigns.flatMap(c => (c.leads || []).map(l => ({ ...l, campaignId: c.id, campaignName: c.name }))).filter(l => l.status === 'convertido').length === 0 ? <tr><td colSpan="5" className="p-12 text-center text-[#9CA3AF] font-medium">Nenhum contrato fechado ainda. Dê baixa em algum lead como "Convertido".</td></tr> : campaigns.flatMap(c => (c.leads || []).map(l => ({ ...l, campaignId: c.id, campaignName: c.name }))).filter(l => l.status === 'convertido').map(lead => (
                    <tr key={lead.id} className="hover:bg-[#18181B] transition-colors">
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="border border-[#27272A] bg-[#09090B] rounded-3xl p-6 space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    { step: '1. Empresas Mapeadas', count: campaigns.reduce((acc, c) => acc + (c.leads?.length || 0), 0), pct: 100, color: 'bg-slate-500' },
                    { step: '2. Abordadas', count: campaigns.reduce((acc, c) => acc + (c.leads?.filter(l => l.status !== 'não contatado').length || 0), 0), pct: campaigns.reduce((acc, c) => acc + (c.leads?.length || 0), 0) > 0 ? Math.round((campaigns.reduce((acc, c) => acc + (c.leads?.filter(l => l.status !== 'não contatado').length || 0), 0) / campaigns.reduce((acc, c) => acc + (c.leads?.length || 0), 0)) * 100) : 0, color: 'bg-[#C5A27D]' },
                    { step: '3. Convertidas (Sucesso)', count: campaigns.reduce((acc, c) => acc + (c.leads?.filter(l => l.status === 'convertido').length || 0), 0), pct: campaigns.reduce((acc, c) => acc + (c.leads?.filter(l => l.status !== 'não contatado').length || 0), 0) > 0 ? Math.round((campaigns.reduce((acc, c) => acc + (c.leads?.filter(l => l.status === 'convertido').length || 0), 0) / campaigns.reduce((acc, c) => acc + (c.leads?.filter(l => l.status !== 'não contatado').length || 0), 0)) * 100) : 0, color: 'bg-emerald-500' }
                  ].map((step, idx) => (
              ...
              <div className="border border-[#27272A] bg-[#09090B] rounded-3xl p-6 space-y-6">
                ...
                {NICHES.map(n => {
                  ...
                  return { name: n.name, total: nicheLeads.length, converted: nicheLeads.filter(l => l.status === 'convertido').length, rate: nicheLeads.length > 0 ? Math.round((nicheLeads.filter(l => l.status === 'convertido').length / nicheLeads.length) * 100) : 0 };
                }).filter(s => s.total > 0).length === 0 ? <p className="text-[#9CA3AF] text-sm font-medium py-12 text-center">Nenhum dado analítico disponível ainda.</p> : (
                  <div className="space-y-4">
                    {NICHES.map(n => {
                      const nicheLeads = campaigns.filter(c => c.niche === n.name).flatMap(c => c.leads || []);
                      return { name: n.name, total: nicheLeads.length, converted: nicheLeads.filter(l => l.status === 'convertido').length, rate: nicheLeads.length > 0 ? Math.round((nicheLeads.filter(l => l.status === 'convertido').length / nicheLeads.length) * 100) : 0 };
                    }).filter(s => s.total > 0).map((sn, idx) => (
        ...
        {/* VIEW: DETALHES DE UMA CAMPANHA */}
        {currentView === 'campaign' && campaigns.find(c => c.id === activeCampaignId) && (
          <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center pb-4 border-b border-[#27272A]">
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => exportCampaignToCSV(campaigns.find(c => c.id === activeCampaignId))} className="inline-flex items-center gap-2 text-xs font-bold text-slate-300 bg-[#18181B] hover:bg-[#27272A] px-4 py-2.5 rounded-xl border border-[#27272A] transition-all shadow-sm"><Download size={14} /> Exportar CSV</button>
                <button onClick={() => deleteCampaign(activeCampaignId)} className="inline-flex items-center gap-1.5 text-xs font-bold text-rose-500 bg-[#18181B] border border-[#27272A] hover:bg-rose-500/10 px-4 py-2.5 rounded-xl border border-white/5 transition-all"><Trash2 size={14} /> Excluir Fluxo</button>
              </div>
            </div>
            ...
            <div className="border border-[#27272A] bg-[#09090B] rounded-3xl p-8 flex justify-between items-center">
              <div>
                ...
                <h1 className="text-2xl font-bold text-white mt-1">{campaigns.find(c => c.id === activeCampaignId).name}</h1>
                <div className="flex gap-2 mt-4 text-xs font-semibold text-slate-300">
                  <span className="bg-[#18181B] border border-[#27272A] px-3 py-1.5 rounded-lg">{campaigns.find(c => c.id === activeCampaignId).niche}</span>
                  <span className="bg-[#18181B] border border-[#27272A] px-3 py-1.5 rounded-lg flex items-center gap-1"><MapPin size={12} /> {campaigns.find(c => c.id === activeCampaignId).location}</span>
                  <span className="bg-[#18181B] border border-[#27272A] px-3 py-1.5 rounded-lg text-[#C5A27D]">{(campaigns.find(c => c.id === activeCampaignId).leads || []).length} leads mapeados</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {(campaigns.find(c => c.id === activeCampaignId).leads || [])
                .filter(lead => {
                    ...
                    <div key={lead.id} className="border border-[#27272A] bg-[#09090B] rounded-3xl p-8 flex flex-col md:flex-row gap-8">
                      <div className="flex-1 space-y-6">
                        <div>
                          ...
                          <a href={lead.link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#C5A27D] hover:text-[#b5926d] mt-1">Analisar Origem / Site <ExternalLink size={12} /></a>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {(lead.weaknesses || []).map((w, i) => (
                              <div key={i} className="flex justify-between items-center p-3 bg-[#18181B] border border-[#27272A] rounded-xl text-xs text-slate-300 font-semibold"><span>{w.description}</span><span className={`inline-flex items-center px-2 py-0.5 text-[10px] font-bold rounded-full ${w.severity === 'high' ? 'bg-rose-500/10 text-rose-500 border border-rose-500/20' : 'bg-amber-500/10 text-amber-500 border border-amber-500/20'}`}>{w.severity === 'high' ? 'Crítico' : 'Médio'}</span></div>
                            ))}
                          </div>
                        </div>

                        {/* Mensagem Editável de Abordagem para cada Lead */}
                        <div className="space-y-2">
                          <div className="flex justify-between items-center">
                             <span className="text-xs uppercase font-bold text-slate-500 tracking-wider flex items-center gap-1.5">
                               <MessageSquare size={14} className="text-[#C5A27D]" /> Mensagem de Abordagem
                             </span>
                          ...
                          <textarea
                            ...
                            value={lead.pitch || ''}
                            onChange={(e) => updateLeadCrmField(activeCampaignId, lead.id, 'pitch', e.target.value)}
                            placeholder="Escreva a mensagem personalizada para este cliente..."
                          />
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <textarea 
                            ...
                            value={lead.notes || ''}
                            onChange={(e) => updateLeadCrmField(activeCampaignId, lead.id, 'notes', e.target.value)}
                          />
                        </div>
                      ...
                      {/* Canais de Contato e Status do CRM */}
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            ...
                            {getWhatsAppLink(lead.contact_phone, lead.pitch) && (
                              <a href={getWhatsAppLink(lead.contact_phone, lead.pitch)} target="_blank" rel="noreferrer" className="w-full py-2.5 bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 text-emerald-500 font-bold rounded-xl text-xs text-center flex items-center justify-center gap-2 transition-all">
                          ...
                          <div className="border border-[#27272A] bg-[#18181B]/50 p-4 rounded-xl space-y-3">
                            ...
                            <div className="space-y-2">
                              <div>
                                ...
                                <input type="date" className="w-full bg-[#09090B] border border-[#27272A] text-slate-300 text-xs px-2.5 py-1.5 rounded-lg focus:outline-none focus:border-[#C5A27D]" value={lead.approachDate || ''} onChange={(e) => updateLeadCrmField(activeCampaignId, lead.id, 'approachDate', e.target.value)} />
                              </div>
                              {lead.status === 'convertido' && (
                                <>
                                  <div>
                                    <label className="text-[10px] text-slate-400 font-semibold block mb-1">Data da Baixa (Fechamento)</label>
                                    <input type="date" className="w-full bg-[#09090B] border border-[#27272A] text-[#C5A27D] text-xs px-2.5 py-1.5 rounded-lg focus:outline-none focus:border-[#C5A27D]" value={lead.conversionDate || ''} onChange={(e) => updateLeadCrmField(activeCampaignId, lead.id, 'conversionDate', e.target.value)} />
                                  </div>
                                  <div>
                                    <label className="text-[10px] text-slate-400 font-semibold block mb-1">Valor do Fechamento (R$)</label>
                                    <input type="number" className="w-full bg-[#09090B] border border-[#27272A] text-emerald-400 text-xs px-2.5 py-1.5 rounded-lg font-bold focus:outline-none focus:border-[#C5A27D]" value={lead.dealValue || ''} onChange={(e) => updateLeadCrmField(activeCampaignId, lead.id, 'dealValue', Number(e.target.value))} />
                                  </div>
                                </>
                              )}
                            </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {['não contatado', 'contatado', 'convertido', 'perdido'].map(st => {
                              ...
                              return (
                                <button key={st} onClick={() => updateLeadStatus(activeCampaignId, lead.id, st)} className={`px-4 py-3 rounded-xl border text-xs font-bold text-left transition-all flex items-center justify-between ${isActive ? colors[st] + ' border' : 'bg-[#18181B] border-[#27272A] text-slate-500 hover:text-slate-300 hover:bg-[#27272A]'}`}>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 z-[100]">
          <div className="bg-[#09090B] border border-[#27272A] rounded-3xl w-full max-w-md p-6 space-y-6 shadow-2xl">
            <div className="flex justify-between items-center border-b border-[#27272A] pb-4">
              <h3 className="text-lg font-bold text-white font-sans">Editar Perfil de Administradora</h3>
